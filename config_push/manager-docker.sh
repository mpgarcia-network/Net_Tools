...
load_env() {
    ...
    if [ -f "$ENV_FILE" ]; then
        while IFS= read -r line || [ -n "$line" ]; do
            ...
            key=$(echo "$key" | tr -d ' ')
            case "$key" in
                APP_NAME|BRAND_LOGIN|BRAND_APP|APP_PORT|SECRET_KEY|SESSION_SECRET|SESSION_MAX_AGE|SESSION_HTTPS_ONLY|ADMIN_USER|ADMIN_PASSWORD|REQUIRE_APPROVAL|TIMEZONE|NETMIKO_MAX_WORKERS|NETMIKO_CONN_TIMEOUT|LIBRENMS_URL|LIBRENMS_TOKEN|RCONFIG_URL|RCONFIG_TOKEN|RCONFIG_VERIFY_TLS|SWARM_NODE_CONSTRAINT|NETTOOLS_ENV_PATH)

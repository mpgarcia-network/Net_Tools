...
> automatizado no host (via cron).

## Como funciona

- Script: **`scripts/lnms-scan.sh`** (versionado no repo).
- Cron (host, root): **`30 3 * * *`** → roda todo dia às **03:30**.
- Log: **`/var/log/net-tools-lnms-scan.log`** (mantém as últimas 2000 linhas;
  configurável por `LNMS_SCAN_LOG`).
...
```bash
scripts/setup-ativos.sh topology
```

Ou, pelo assistente completo (communities, redes, scan, topologia):
```bash
scripts/setup-ativos.sh          # menu
scripts/setup-ativos.sh scan "10.172.92.0/24"
```
